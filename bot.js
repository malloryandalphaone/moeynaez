...
editedcodes.on("message", (edited) => {
  ...
  if (edited.content === '-new') {
        ...
        if (!edited.guild.roles.exists("name", "Support")) return edited.channel.send(`**يجب عمل رتبة بأسم \`Support\`**`);
        ...
        edited.guild.createChannel(`just-ticket`, "text").then(c => {
            let edited1 = edited.guild.roles.find("name", "Support");
    ...
    if (edited.content === '-close') {
        ...
        edited.channel.send(`**هل أنت متأكد؟ ،\n أكتب __نعم__ لتأكيد**.`)
            .then((m) => {
                edited.channel.awaitMessages(response => response.content === 'نعم', {
                    ...
                    .catch(() => {
                        m.edit('وقت الأغلاق أنتهى , لن تمسح التذكرة .').then(m2 => {
                            m2.delete();
                        }, 3000);
                    });
            });
